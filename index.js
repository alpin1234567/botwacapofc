...
const { help } = require('./alpinofc/help')
const { rules } = require('./alpinofc/rules')
const { bahasa } = require('./alpinofc/bahasa')
const { donasi } = require('./alpinofc/donasi')
const { limits } = require('./alpinofc/limit')
...
async function starts() {
	...
	fs.existsSync('./alpinofc.json') && client.loadAuthInfo('./alpinofc.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./alpinofc.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const alpinofc = ["6289690719275@s.whatsapp.net"]
     ...
     const bayarLimit = (sender, amount) => {
		...
		case 'daftar':
			...
			case 'menu':
                ...
                setTimeout( () => {
					costum(help(prefix, instagram, yt, name, pushname2, user, limitt), text, alpinofc, replySet)
					}, 15000)
					setTimeout( () => {
            ...
            break  
                 ...
                 case 'kalkulator':
				   ...
				   if (isLimit(sender)) return reply(limits.limitend(pushname2))
				    ...
				    break 
				case 'owner':
                 client.sendMessage(from, {displayname: "alpinofc", vcard: vcard}, MessageType.contact, { quoted: mek})
...
//akhir kreator
		    ...
		    case 'darkjokes':
				...
				if (!isUser) return reply(mess.only.userB)
				reply(mess.wait)
				 data = fs.readFileSync('./alpinofc/drak.js');
		    ...
		    case 'map':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *alpinofc BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
          ...
          case 'snack':
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (anu.error) return reply(anu.error)
                ...
                bufferddd = await getBuffer('https://raw.githubusercontent.com/AZIZAE-official/AZIZAE/main/src/glitchtext.png')
                 reply(mess.wait)
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[alpinofc]','red'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
